...
Platform pltDon, pltWind;
int nDx, nDy, nSide, nSide2, nScroll = 1; // 1 for top, 2 for bottom, 3 for side
boolean canJump = true;

void setup () {
  size (800, 600);
  pltDon = new Platform("hair.jpg", 250, 350, 100, 150);
  pltWind = new Platform("wind.jpg", 400, 200, 100, 150);
  sprMario = new Sprite("Mario.jpg", 0, 400, 100, 100);
  frame.requestFocus();
  bg = new Bg("marioBack.jpg", 800, 600);
}
...
void draw() {
  ...
  if (nSide2 == 1) {
    ...
    sprMario.bumpHead();
  } else if (nSide2 == 3 && sprMario.nDx >0) {
    sprMario.backUpX();
  } else if (nSide2 == 4 && sprMario.nDx <0) {
    sprMario.backUpX();
  }
}
