class Enemy {
  ...
  Enemy(String _sFile, float _fX, float _fY, int _nH, int _nW, float _fVelocity, float _fVelocityMax, float _fAccel, int _nDirec, int _nSpeed) {
    ...
    // added this
    wallTop = new Wall("nothing.png", int(vPos.x), int(vPos.y), nW, 10);
    wallBot = new Wall("nothing.png", int(vPos.x), int(vPos.y+ nH -10), nW, 10);
    wallLeft = new Wall("nothing.png", int(vPos.x), int(vPos.y), 10, nH);
    wallRight = new Wall("nothing.png", int(vPos.x + nW -10), int(vPos.y), 10, nH);
  }
  // ================================= UPDATE =========================
  public void update() {
    fXStart = vPos.x;
